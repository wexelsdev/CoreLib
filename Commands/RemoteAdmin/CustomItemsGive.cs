...
namespace CoreLib.Commands.RemoteAdmin
{
    ...
    public class CustomItemsGive : AutoPermCheckCommand, IUsageProvider
    {
        protected override bool ExecutePermitted(ArraySegment<string> arguments, ICommandSender sender, out string response)
        {
            ...
            switch (arguments.Count)
            {
                case 1:
                    ...
                    if (CustomItems.API.Features.CustomItem.Registered.FirstOrDefault(x => x.Id == id) == null)
                    {
                        response = "Броу, с тобой всё нормально? Такого предмета нету, напиши customitem list";
                        return false;
                    }

                    CustomItems.API.Features.CustomItem.Registered.First(x => x.Id == id).Give(player);
        ...
        public override string Description => "Выдаёт вам кастомный предмет";
